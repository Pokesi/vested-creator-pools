...
contract iVCP {
  ...
  function calculatePayout(
    ...
    uint numerator = 1;
    uint denominator = 5;
    if (contributed/totalDeposited >= (numerator/denominator)) {
  ...
  function payout(
    ...
    if (!((_epoch * interval_time) < block.timestamp)) {
      ...
      uint base = calculatePayout(contributed[epoch][creator], address(this).balance, total_deposited[epoch]);
      (uint256 fee, uint256 payout) = (base/200, (199*base)/200);
